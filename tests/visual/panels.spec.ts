 ...
 * are DOM text, and no two font stacks rasterise text identically, so the
 * pinned container is the single source of truth (it is also exactly where
 * CI compares them). A host run skips them instead of failing on fonts it
 * never promised to have.
 */
...
async function boot(page: Page): Promise<void> {
  ...
  await expect(page.locator('html')).toHaveAttribute('data-render-state', 'ready', { timeout: 30_000 });
...
test.describe('panel goldens — Phase 18', () => {
  ...
  test('settings', async ({ page }) => {
    ...
    await expect(page.getByTestId('audio-settings')).toHaveScreenshot('panel-settings.png');
  });

  test('staff', async ({ page }) => {
    await boot(page);
    await page.getByTestId('dock-staff').click();
    await expect(page.getByTestId('staff-panel')).toHaveScreenshot('panel-staff.png');
  });

  test('analytics, empty state', async ({ page }) => {
    await boot(page);
    await page.getByTestId('dock-analytics').click();
    await expect(page.getByTestId('analytics-panel')).toHaveScreenshot('panel-analytics-empty.png');
  });
  ...
  test('dock', async ({ page }) => {
    await boot(page);
    await expect(page.locator('.action-dock')).toHaveScreenshot('panel-dock.png');
  });
});
