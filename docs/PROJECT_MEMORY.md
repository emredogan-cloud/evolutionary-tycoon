...
| 2026-08-19 | **Y**      | **Konsolidasyon tamam.** Final ağaçta `pnpm verify` (1374 test; bench kapısı yük ~5'te bir kez yanlış tetiklendi, yük 1.5'te 21/21) + E2E 146+12 skip + WebKit (konteyner, hermetik, 3/3) + 14 golden + iki balance konfigürasyonu yeşil. Preview deploy SHA birebir (`a2596421…`/`bh7uneeeb`), CDN'de 4 aşama 171 kare 0 placeholder, konsol 0 hata, soğuk yükleme 5.3 s. CDN'e karşı tam süit masaüstünden güvenilir sinyal vermiyor — altı boot helper'daki HUD-köprü yarışı bulundu ve kapatıldı (`e331158`; sim dünya hash'i ilerlerken HUD 0.00'da kalıyordu), bağlayıcı hüküm CI `preview-e2e`. Dal push edildi; kanıt eki commit'i CI sonuçlarını taşıyor; raporlar yazıldı; P14 BAŞLATILMADI.                                                                                                                                                |
